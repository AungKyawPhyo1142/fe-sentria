...
const storageWithExpiry: PersistStorage<PersistedAlerts> = {
  getItem: (name) => {
    ...
    try {
      ...
      if (parsed.timestamp && now - parsed.timestamp > EXPIRY_TIME) {
        ...
        console.log(`[ZustandSocket] Storage item "${name}" has expired.`)
        return null
      }
...
export const useSocketStore = create<SocketState>()(
  persist(
    (set, get) => ({
      ...
      allEarthquakeAlerts: [],
      addEarthquakeAlertToList: (alert) => {
        set((state) => ({
          allEarthquakeAlerts: [alert, ...state.allEarthquakeAlerts],
        }))
      },
      ...
      connect: () => {
        ...
        console.log(
          `[ZustandSocket] Creating new connection to: ${SOCKET_SERVER_URL}`,
        )
      ...
      earthquakeAlertListener: () => {
        ...
        socket.on('earthquake_alert', (payload) => {
          ...
          //   }
          //to reuse alertData
          const alertData = payload?.notificationData?.data
          if (alertData) {
            set({ latestEarthquakeAlert: alertData })
            get().addEarthquakeAlertToList(alertData)
            console.log('update lateseEQ: ', alertData)
          } else {
      ...
      setupReportUpdateListener: (queryClient) => {
        ...
        socket.on(
          'report_factcheck_update',
          (data: ReportFactCheckUpdatePayload) => {
            console.log(
              '[ZustandSocket] Received "report_factcheck_update":',
              data,
            )
            if (data && data.reportId && data.factCheck) {
              ...
              queryClient.setQueryData<InfiniteData<ReportResponse>>(
                ...
                (oldData) => {
                  if (!oldData) {
                    console.warn(
                      `[ZustandSocket] No data found in cache for query key:`,
                      queryKey,
                    )
                    return oldData
                  }
    ...
    }),
    {
      ...
      partialize: (state) => ({
        allEarthquakeAlerts: state.allEarthquakeAlerts,
        latestEarthquakeAlert: state.latestEarthquakeAlert,
      }),
    },
  ),
)
